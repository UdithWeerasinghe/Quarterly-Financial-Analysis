...
import { ratioColors } from '../../utils/colors';
import ExportBar from '../ExportBar';
import DrilldownModal from '../DrilldownModal';  // adjust path if needed
...
const RatioChart = ({ data, ratio, isAnnual, yearRange, yearOptions, quarterRange, quarterOptions, darkMode }) => {
    ...
    datasets: [
      {
        ...
        borderColor: ratioColors[ratio] || '#43a047',
        backgroundColor: (ratioColors[ratio] || '#43a047') + '33',
        borderWidth: 3,
        tension: 0.1,
        fill: true,
      },
  ...
  const options = {
    ...
    plugins: {
      ...
      title: { display: true, text: `${ratio} Over Time`, color: axisColor },
      tooltip: {
        enabled: true,
  ...
  return (
    <Box sx={{ mt: 2, px: isMobile ? 0.5 : 2, width: '100%', overflowX: isMobile ? 'auto' : 'visible' }}>
      <ExportBar chartRef={chartRef} data={filteredData} fileName={ratio} />
